...
#include "../../../modules/task_2/maximova_i_allreduce/allreduce.h"
#include <cmath>
#include <cstring>
#include <stdexcept>
...
int Allreduce(void* sendbuf, void* recvbuf, int count, MPI_Datatype datatype,
  ...
  if (sendbuf == nullptr)
    throw std::runtime_error("Send buffer points to nullptr.");
  if (recvbuf == nullptr)
    throw std::runtime_error("Receive buffer points to nullptr.");
  if (count == 0) throw std::runtime_error("Send buffer is empty.");
